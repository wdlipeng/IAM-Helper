Log Collector is a tool that helps to collect logs of all IAM DPs in defined time range on all IAM nodes. It also help packed the log into a compressed file and 1st line support engineer can easily sent the file to 2nd line support for analyze when needed.
...
4, To start collecting the log, execute "python log-collector.py -t <time interval> -u <username> -p <password>", here:
	-t: Defines the interval in minutes for log collection. If not specified, by default the tool will collect all logs.
